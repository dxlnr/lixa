...
const Sidebar = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const response = await fetch('http://127.0.0.1:5000/api/copilot/prompt', {
          method: 'POST', 
          headers: {
        ...
        <div class="flex flex-col">
          <div class="flex-none">
            <div class="flex flex-row">
              <button type="button" class="bg-white text-black hover:bg-slate-400 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-black-800 shadow-lg shadow-black-500/50 dark:shadow-lg dark:shadow-black-800/80 font-medium rounded-md text-md px-5 py-2.5 text-center mr-2 mb-2">
              ...
              <form onSubmit={handleSubmit}>
                <input type="text" id="input-group-1" class="w-fit placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm border-0 shadow-lg outline-none focus:outline-none focus:ring ring-slate-400 px-3 py-3 h-12 mr-2" placeholder="prompt" value={prompt()} onInput={(e) => setPrompt(e.target.value)}/>
            </form>
            </div>
          </div>
           <div class="flex-none">
